...
class ReduxFramework_select_image extends ReduxFramework {
    ...
    function render() {
        ...
        if (!empty($this->field['options'])) {
            ...
            // Process placeholder
            $placeholder = (isset($this->field['placeholder'])) ? esc_attr($this->field['placeholder']) : esc_html__('Select an item', 'consultingpress');
            ...
            echo '<strong>' . esc_html__('No items of this type were found.', 'consultingpress') . '</strong>';
        }
    }
...
//function
    ...
    function enqueue() {
        wp_enqueue_script(
                'field-select-image-js', VOLCANNO_TEMPLATEURL . '/core/components/theme-options/custom-fields/select_image/field_select_image.js', array(), time(), true
        );

        wp_enqueue_style(
                'redux-field-select-image-css', VOLCANNO_TEMPLATEURL . '/core/components/theme-options/custom-fields/select_image/field_select_image.css', array('select2-css'), time()
        );
    }
